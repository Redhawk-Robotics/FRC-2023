...
import frc.robot.constants.Setting.LimelightConstants;
...
public class AlignWithGridApril extends CommandBase {
  ...
  public AlignWithGridApril(LimelightAprilTag limelight, SwerveSubsystem drive) {
    m_drive = drive;
  ...
  public void initialize() {
    ...
    m_thetaController = new PIDController(LimelightConstants.GAINS_ANGLE_SNAP.kP*2, LimelightConstants.GAINS_ANGLE_SNAP.kI, LimelightConstants.GAINS_ANGLE_SNAP.kD);
  ...
  public void execute() {
    ...
    rotationVal = MathUtil.clamp(rotationVal, -Setting.maxAngularVelocityRadiansPerSecond * 0.4, Setting.maxAngularVelocityRadiansPerSecond * 0.4);

    m_pidControllerX.setSetpoint(LimelightConstants.ALIGNED_GRID_APRIL_X);
    xTrans = m_pidControllerX.calculate(m_limelight.getXRear());
    xTrans = MathUtil.clamp(xTrans, -0.5, 0.5);

    m_pidControllerY.setSetpoint(LimelightConstants.ALIGNED_GRID_APRIL_AREA);
    yTrans = m_pidControllerY.calculate(m_limelight.getAreaRear());
